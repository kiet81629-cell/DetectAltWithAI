...
const BLACKLIST_BUTTON_ID = "blacklist_button";
...
export async function sendAlarmEmbed(
    ...
    const blacklistButton = createButton(
    ...
    const actionFilter = (interaction) =>
        (interaction.customId === BAN_BUTTON_ID || interaction.customId === BLACKLIST_BUTTON_ID) &&
    ...
    handleActionCollector(manager, message, collector, suspectedAs);
}

function handleActionCollector(
    manager: NlpManager,
    ...
    collector.on("collect", async (interaction) => {
        if (interaction.customId === BLACKLIST_BUTTON_ID) {
            const roleId = "1490717283327283230";
            try {
                await originalMessage.member?.roles.add(roleId);
                await interaction.reply({
                    content: `Successfully added ${originalMessage.author.tag} to blacklist and assigned the role.`,
                    ephemeral: true
                });
            } catch (error) {
                await interaction.reply({
                    content: "Failed to assign role. Ensure the bot has 'Manage Roles' permissions and its role is positioned higher than the target role.",
                    ephemeral: true
                });
            }
            return;
